...
class Board2(QFrame):
    ...
    keyPressevent4 = QtCore.pyqtSignal()
    keyPressevent5 = QtCore.pyqtSignal()
    keyPressevent6 = QtCore.pyqtSignal()
    ...
    def removeVrag(self):
        if self.vrag is not None:
            vrag_row = self.vrag.vrow
            vrag_col = self.vrag.vcol
            self.vrag = None
            self.score += 1000
            self.en += 1
            # Удалить клетку, на которой находился враг, из списка кирпичных стен
    ...
    def drawVrag(self, painter):
        x = (self.width() - self.num_cols * self.cell_size) // 2 + self.vrag.vcol * self.cell_size
        y = (self.height() - self.num_rows * self.cell_size) // 2 + self.vrag.vrow * self.cell_size
        self.vrag_texture = QPixmap("vrag.png")
        painter.drawPixmap(QRect(x, y, self.cell_size, self.cell_size), self.vrag_texture)
    ...
    def moveVrag(self):
        if self.vrag is not None:
            ...
            for direction in directions:
                next_row = self.vrag.vrow + direction[0]
                next_col = self.vrag.vcol + direction[1]
                ...
                if (0 < next_row < self.num_rows - 1 and 0 < next_col < self.num_cols - 1) and \
                    ...
                    valid_directions.append(direction)
                if self.vrag is not None:
                    if (self.vrag.vrow, self.vrag.vcol) == (self.player.row, self.player.col):
                        self.pause()
                        self.gameOver()
                        print("moveVrag")
                        return
            ...
            if valid_directions:
                direction = random.choice(valid_directions)  # Случайный выбор направления из доступных
                self.vrag.vrow += direction[0]
                self.vrag.vcol += direction[1]
    ...
    def opendoor(self):
        ...
        if self.open_door == True:
            ...
            if reply == QMessageBox.Yes:
                    self.keyPressevent5.emit()
            elif reply == QMessageBox.No:
                    self.keyPressevent6.emit()
...
class Bomb:
    ...
    def explode(self, inner_walls):
        ...
        if self.board.vrag is not None:
            ...
            if (vrag.vrow, vrag.vcol) in [(cell.row, cell.col) for cell in explosion_cells]:
